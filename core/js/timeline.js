...
jQuery.fn.timelinr = function(options){
	...
	$(function(){
		...
		$(settings.issuesDiv+' li').css({"width": widthContainer});
		...
		if(settings.orientation == 'horizontal') {	
			...
			$(settings.datesDiv).width(widthDate*howManyDates).css('marginLeft', widthContainer/3);
...
$(window).resize(function(){
		...
		howManyDates = $(settings.datesDiv+' li').length;
	...
	widthIssue = $(settings.issuesDiv+' li').width();
	 ...
	 widthDate = $(settings.datesDiv+' li').width();
		...
		testando = $(settings.issuesDiv).find('.selected').index()

	$(settings.issuesDiv).animate({'marginLeft':-widthIssue*testando},{queue:false, duration:settings.issuesSpeed});
	$(settings.datesDiv).animate({'marginLeft':-(widthDate*testando-widthContainer/3)},{queue:false, duration:'settings.datesSpeed'});
});
